...
- A Runeweave OS loop that repeatedly runs Runesmith Next over engine-owned cards until work is sealed or the runtime reaches a concrete stop condition: idle, needs implementation evidence, failed proof, Faultline architecture review, risk hold, blocker, or safety step limit.
...
- `runesmith_os_run`: run Runeweave until the active mission is sealed or stopped by implementation work, failed proof, Faultline architecture review, unresolved risk, blocker, idle state, or safety limit.
- `runesmith_next`: run the active Runebook card through proof execution, repair proof, recovery, supplied Faultline or risk decision, or normal loop advancement.
- `runesmith_autopilot_tick`: advance the active task through the evidence gate, surface repair diagnostics when verification failed, surface Faultline when repeated repairs fail, hold unresolved risk for a decision, and complete the task when the contract is satisfied.
