...
- A Runesmith Plan Refinery that lets OpenCode remap a thin Covenant mission into ordered proof-backed slices, records the planning decision, persists the capsule, and re-enters the shared loop so independent ready slices are claimed automatically. Its default Pathfinder Goal Weave profiles the mission text and emits install, runtime, UI, repair, docs, or focused slices when the user has not provided a task graph.
...
- A Runesmith Repair Contract that reads failed diagnostics and later implementation edits, classifies repair as awaiting scoped edit, ready for focused proof, over-broad, proven, or Faultline, and surfaces that contract across OpenCode, CLI, dashboard, Review Lens, and Seal Audit.
- A Faultline breakpoint that detects three failed proof diagnostics without passing proof and switches OpenCode, CLI, and dashboard surfaces from ordinary repair into architecture review.
...
- A Runesmith Seal Audit that combines Proof Plan, Scope Sentinel, Redline Proof, Repair Contract, Review Lens, mission state, and Sealmark decision evidence into one engine-owned completion gate across OpenCode, CLI, and dashboard surfaces.
- A Runesmith Decision Guard that refuses to synthesize autonomous Review or Seal decision evidence while Review Lens or Seal Audit has critical or blocking findings, returns a `decision-held` state, and stops Runeweave with the exact guard reason instead of spinning or claiming false completion. Loop Pulse and Mission Memory re-derive that guard from the persisted capsule so reloads still show the blocker.
- A Runesmith Next router that executes the active Runebook card from one OpenCode tool, one CLI command, and one dashboard action, selecting proof, repair, recovery, supplied Faultline or risk decision, or loop advancement from runtime state.
- A Runeweave OS loop that repeatedly runs Runesmith Next over engine-owned cards until work is sealed or the runtime reaches a concrete stop condition: idle, needs implementation evidence, failed proof, Faultline architecture review, risk hold, Decision Guard hold, blocker, or safety step limit.
...
- Plan Refinery mutation for fresh missions, including task-plan validation, active root lease preservation, planning decision evidence, and immediate loop advancement.
...
- `runesmith mission inspect <id>`: print graph, Loop Pulse, Mission Map, Plan Contract, Dispatch Matrix, Scope Sentinel, Redline Proof, Repair Contract, Review Lens, Seal Audit, Runebook card, Proof Plan, missing proof, active diagnostics, active runes, evidence, leases, and recovery state.
...
- Repair Gate: turn failed verification diagnostics into hypothesis-first, one-variable repair work before another proof attempt.
- Faultline Breakpoint: stop repeated failed repairs and question the architecture before another patch.
...
The runtime also derives a live Runesmith Control Brief from the current snapshot. This brief does not ask the user to run a skill. It tells the coding agent what stage comes next, which mission and task are active, what proof is required, which evidence is still missing, and which risks are unresolved. Failed or unknown test runs are treated as diagnostics, so the brief moves the task into Repair Gate and keeps the agent focused on the latest failing command until passing test proof exists. Repair Gate now requires a falsifiable hypothesis and one repair variable before the exact failing command is rerun, while Repair Contract turns that requirement into an evidence-derived status instead of prompt-only advice. If three failed proof diagnostics accumulate without passing proof and the latest evidence is still diagnostic, the brief moves into Faultline Breakpoint instead of allowing a fourth blind patch. Unresolved risk evidence moves the task into Mirror Review with a critical decision hold.

The Control Brief also includes Runesmith-native runes. The Runebook converts those runes and the live Loop Pulse into a concrete procedure card selected from runtime state, such as `Forge Trace implementation loop` during scoped edits, `Proofwright proof gate` when evidence is missing, `Faultwright repair loop` when verification fails, `Faultline architecture breakpoint` when repeated repairs fail, or `Mirrorglass risk decision` when risk is newer than decision evidence. Each card carries autonomy mode, trigger, intent, steps, evidence requirements, exact Proof Plan commands, OpenCode tool hints, and stop conditions. Forge Trace cards are proof-first for testable behavior: create or update focused proof before production edits, then make the smallest scoped change that satisfies it. Faultwright cards are hypothesis-first for failed proof: acknowledge the diagnostic, state the repair hypothesis, change one variable, and rerun the exact failing command. Repair Contract then checks whether the ledger actually shows a scoped repair edit and focused proof instead of broad guessing. Faultline cards compare the repeated diagnostics and repairs, name the broken assumption or boundary, then choose redesign, revert, scope split, or a new hypothesis before proof is retried. This borrows the useful discipline of explicit workflows while keeping the user experience install-once and automatic; users should not need to invoke external skills or remember process names.

The Protocol Deck sits beside the Runebook as Runesmith's own workflow-method layer. It derives one active protocol from Loop Pulse and Proof Plan, such as `Forge Trace Protocol`, `Proofwright Proof Protocol`, `Faultwright Repair Protocol`, `Faultline Breakpoint Protocol`, `Recovery Loom Protocol`, or `Mirrorglass Risk Protocol`. Each protocol carries objective, procedure, verification, forbidden moves, and tool hints. The Faultwright protocol explicitly forbids symptom patches that are not linked to the active diagnostic, and Repair Contract reports whether the subsequent evidence honored that rule. The Faultline protocol forbids stacking another patch on the same hypothesis. This is where Runesmith internalizes the useful concept of explicit agent methods while keeping the user experience direct: users install Runesmith once, and the engine selects the protocol automatically.
...
Review Lens sits beside Mission Map as the pre-seal review surface. It derives a checklist from the runtime capsule: implementation evidence, scope health, proof freshness, Redline Proof ordering, Repair Contract status, unresolved risk state, and review decision state. If Redline Proof is missing, it produces an advisory finding; if Repair Contract shows an over-broad repair, it produces a warning finding; if proof is missing, risk is unresolved, or Scope Sentinel reports critical drift, it holds review as waiting or blocked. When proof, risk, and scope gates are clear, it marks the mission ready for Mirror Review. Mirrorglass review is findings-first: blocking findings lead, approval comes only after they are resolved or recorded as explicit risk evidence, and autonomous review decisions include a compact Review Lens summary in their decision evidence. Decision Guard enforces that promise in the state machine: if Review Lens is not ready, the loop returns `decision-held`/missing `decision` instead of auto-approving.

Seal Audit sits above Review Lens as the final completion-claim gate. It derives mission-state, proof-gate, redline-gate, repair-gate, scope-gate, review-gate, and seal-decision checks from the same runtime capsule. `collecting-proof` means the agent must run the Proof Plan before claiming done. `blocked` means scope, review, mission, or repair findings must be resolved first. `ready` means Sealmark can record the final checkpoint. `sealed` means the shared loop already produced completion evidence. Decision Guard also checks Seal Audit before generating Sealmark evidence, so even a manually approved Review cannot seal a mission while final audit blockers remain. This internalizes the useful "verification before completion" discipline while keeping the user experience automatic.

The Loop Pulse sits beside the Control Brief. It converts the live runtime state into one next action such as `Wait for goal`, `Continue forge`, `Capture proof`, `Repair diagnostic`, `Resolve risk`, `Recover stale work`, `Review change`, or `Seal mission`. It also derives an execution plan from that action, with active, queued, and blocked steps, required evidence, and the runes that should guide each step. If Decision Guard would block autonomous Review or Seal, Pulse promotes that persisted state into `Resolve review guard` or `Resolve seal guard` with the guard findings in blockers. OpenCode prompt injection, compaction context, CLI status, and the dashboard should all show this same pulse so the OS has one source of truth for what the agentic loop should do next.

Mission Memory sits above the pulse as the durable continuation layer. It classifies the mission as idle, active, blocked, needs-proof, needs-repair, needs-architecture, needs-recovery, or sealed; summarizes active task, open and completed task counts, latest change evidence, passing proof, diagnostics, and decisions; and produces one handoff sentence that can survive OpenCode restart, compaction, CLI inspection, or dashboard reload. Repair handoffs name the latest diagnostic and carry the hypothesis-first next move so restarts do not collapse back into blind retries. Faultline handoffs preserve the repeated-failure breakpoint so resumed sessions question architecture instead of continuing with another local patch. Decision Guard handoffs preserve the held Review or Seal guard instead of degrading to a generic missing-decision prompt. This is how Runesmith internalizes the useful continuity discipline of workflow systems without asking users to install or invoke a separate process.

Proof Plan sits above Mission Memory as the automatic verification recipe. It detects missing `test-result` proof, stale passing proof, failed diagnostics, and changed-file impact. It reruns the latest failing command first during repair, reruns the last stale targeted proof command after newer edits invalidate it, prepends Runescope impacted tests from changed test files or repository-known source-to-test neighbors, then asks for the repository's typecheck, lint, test, and build scripts when those scripts exist. When Worker Dispatch has a focused packet, Proof Plan targets that packet's task and includes the worker metadata so parallel proof does not drift to another running slice. Faultwright supplies the debugging discipline around that recipe: form one falsifiable hypothesis, change one repair variable, and only then prove the hypothesis with the captured command. Repair Contract supplies the runtime audit around that recipe: one implementation edit after diagnostic is ready for focused proof, multiple implementation edits before proof are over-broad, and passing focused proof closes the contract. Faultline keeps the latest failing command in the recipe but changes the active method so proof is not retried until the architecture question has a new answer. The plan is derived from runtime state, package metadata, changed-file evidence, and repository file discovery, so users get one install-once orchestration loop instead of a separate checklist they need to remember.

Proof Runner executes the active recipe. It is harness-independent in `packages/core`: callers provide a command runner, evidence ids, and a clock, while the core runner converts command outcomes into task evidence. If the recipe is worker-focused, the resulting `test-result` or `diagnostic` evidence carries packet, agent, holder, and lease metadata. OpenCode, CLI, and dashboard surfaces use that same runner, so a proof run from chat, the terminal, or the control surface writes the same ledger evidence.

Runesmith Next sits above the Runebook and dispatches the active procedure card without exposing the internal command tree to the user. `Pathfinder plan refinery` cards refine thin evidence-free Covenant maps before broad Forge work starts, `Capture proof` and `Repair diagnostic` cards execute Proof Runner, `Review faultline` cards hold until an architecture path is supplied through `runesmith_faultline_resolve` or `runesmith_next`, `Resolve risk` cards hold until a decision summary is supplied and then record decision evidence, recovery cards reclaim stale work, and ordinary advance cards reuse the shared Runic mission loop. When the loop refuses autonomous Review or Seal synthesis, Next reports `decision-held` with the Decision Guard payload so OpenCode, CLI, and dashboard surfaces stop with actionable findings. This is the core product lesson borrowed from workflow skill systems but made native: the discipline is explicit, while the interaction stays install-once and engine-owned.

Runeweave sits above Runesmith Next as the full OS run loop. It repeatedly executes engine-owned cards with a maximum step budget, aggregates proof commands, and returns a final status and stop reason. It is allowed to handle recovery, proof, Faultline and risk decisions when supplied, Review, Seal, and task claiming. It deliberately stops at `Continue forge` because scoped implementation edits are creative work for the coding agent, not a fake state transition. It also stops immediately on `decision-held` so Review/Seal guard findings do not spin until the safety limit. This gives users one magical command while keeping the runtime honest about what it can prove.

The default Covenant task plan is:

- Forge: implementation work requiring `file-change` and passing `test-result` evidence.
- Review: dependent review work requiring `decision` evidence, generated automatically only when verified Forge proof exists and Review Lens has no blocking findings.
- Seal: dependent checkpoint and handoff work requiring `decision` evidence, generated automatically only when Review is complete and Seal Audit has no blocking findings.
...
Plan Refinery is the install-once planning bridge after Mission Ignition. When Plan Contract reports a thin Forge/Review/Seal shell, OpenCode can call `runesmith_plan_refine` with ordered task slices. Normal hands-off surfaces also use it automatically: Loop Pulse selects `Refine plan`, Runebook exposes `Pathfinder plan refinery`, and Runesmith Next/Runeweave execute the refinement while the mission is still evidence-free. The default path is Pathfinder Goal Weave: core profiles the mission text and generates proof-backed install, runtime, UI, repair, docs, or focused implementation slices, so arbitrary projects do not inherit a Runesmith-specific runtime/interface map. The core validates that every slice has evidence requirements, the first slice records a planning decision, and at least one implementation slice is backed by file-change and test-result proof. The runtime remaps the mission only while no task evidence has been recorded, preserves the active root lease, writes a new `mission.mapped` event, records the planning decision evidence, then runs the shared mission loop. If the refined map exposes independent ready implementation slices, Dispatch Matrix and Agent Mesh claim them in the same pass, and Worker Dispatch turns those routed slots into executable packets with agent, model, tool, scope, evidence, lease, claim, and handoff metadata. The same packet claim primitive is exposed as `runesmith_worker_claim`, `runesmith worker claim`, and the dashboard `Claim packet` action so users do not manage mission or task ids. Claiming, replaying, or letting the shared OS loop claim a packet writes a focused worker event, and automatic OpenCode file/shell evidence plus Proof Runner output is attributed to that packet until another worker packet is focused. This internalizes the useful "write the plan before coding" discipline without asking the user to install or invoke a separate planning workflow.
...
The same compaction path appends the live Runesmith Control Brief, Loop Pulse, Mission Map, Plan Contract, Dispatch Matrix, Worker Dispatch, Scope Sentinel, Redline Proof, Repair Contract, Review Lens, Seal Audit, Runebook, Mission Memory, and Proof Plan, so resumed sessions keep the next stage, procedure card, plan quality, routing slots, executable worker packets, scope guard, proof-first ordering, repair contract, proof obligations, completion gate, exact verification commands, and handoff without requiring the user to install or invoke an external workflow.
...
- `runesmith_plan_refine`: replace a thin mission map with proof-backed task slices, record planning decision evidence, persist the capsule, and advance the shared loop so independent ready work is claimed automatically.
- `runesmith_os_run`: run Runeweave until the active mission is sealed or stopped by implementation work, failed proof, Faultline architecture review, unresolved risk, Decision Guard hold, blocker, idle state, or safety limit.
- `runesmith_next`: run the active Runebook card through proof execution, repair proof, recovery, supplied Faultline or risk decision, Decision Guard hold reporting, or normal loop advancement.
- `runesmith_worker_claim`: claim or re-focus the next or selected Worker Dispatch packet through its assigned contract, holder, idempotency key, and lease TTL without raw mission or task ids; routine `runesmith_next`, `runesmith_os_run`, idle Runeweave, and recovery claims also focus their returned packets automatically, and the resulting worker focus steers automatic OpenCode evidence capture and Proof Runner output to that packet.
- `runesmith_autopilot_tick`: advance the active task through the evidence gate, surface repair diagnostics when verification failed, surface Faultline when repeated repairs fail, hold unresolved risk for a decision, hold Review/Seal decision synthesis when guard findings remain, and complete the task when the contract is satisfied.
- `runesmith_proof_run`: execute the active Proof Plan inside OpenCode, record passing commands as `test-result` evidence, record failing commands as `diagnostic` evidence with focused worker metadata when present, and advance the shared mission loop when proof passes.
...
- `runesmith_covenant_status`: report the installed autonomous workflow plus the live Agent Mesh, Control Brief, Loop Pulse, Mission Map, Plan Contract, Dispatch Matrix, Worker Dispatch, Scope Sentinel, Redline Proof, Repair Contract, Review Lens, Seal Audit, Runebook card, Proof Plan, and active runes from runtime state.
...
- Run OS executes Runeweave through the same router used by OpenCode and CLI, making the primary dashboard action prove, repair, stop at Faultline, recover, resolve supplied Faultline and risk decisions, advance, and seal until a stop condition is reached.
- Run Next executes only the active Runebook card when a single bounded step is needed.
- Claim packet leases the next visible Worker Dispatch packet through the same runtime capsule as OpenCode and CLI.
...
- Resolve Faultline records architecture-path decision evidence for the active `Review faultline` Loop Pulse state and returns the capsule to focused repair proof.
...
- The right rail shows the Loop Pulse with health, priority, next action, execution plan, Mission Map, Plan Contract, Dispatch Matrix, Worker Dispatch packets, Scope Sentinel, Redline Proof, Repair Contract, Review Lens, Seal Audit, missing evidence, active runes, active Runebook card, Mission Memory, and Proof Plan commands from the same runtime capsule used by OpenCode.
