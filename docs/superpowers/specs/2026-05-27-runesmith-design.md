...
- `runesmith_worker_claim`: claim or re-focus the next or selected Worker Dispatch packet through its assigned contract, holder, idempotency key, and lease TTL without raw mission or task ids; the resulting worker focus steers automatic OpenCode evidence capture to that packet.
