...
- Run Next executes the active Runebook card through the same router used by OpenCode and CLI, so the primary dashboard action can prove, repair, recover, resolve a supplied risk decision, or advance without choosing a low-level command.
