...
- Runesmith Plan Refinery lets OpenCode turn a thin mission into concrete proof-backed slices through `runesmith_plan_refine`; the engine remaps the mission, records the planning decision, then advances the shared loop so independent implementation slices are claimed by the Agent Mesh without user-managed planning steps.
