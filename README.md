...
- Runesmith Agent Mesh ships install-default contracts for Atlas, Oracle, Artificer, Scout, and Steward, then uses Dispatch Matrix recommendations when claiming Covenant tasks so Forge, Review, Seal, recovery, and release work can land on the right built-in role without user-authored setup.
...
- Runesmith Plan Refinery lets OpenCode turn a thin mission into concrete proof-backed slices through `runesmith_plan_refine`; Runesmith Next and Runeweave also trigger Pathfinder Goal Weave automatically when the Plan Contract is thin and evidence-free, so users can just run the OS and let the engine decompose install, runtime, UI, repair, docs, or focused work before broad Forge starts.
- Runesmith Dispatch Matrix turns the mission graph, active leases, and registered agent contracts into an automatic routing surface for serial work, parallel-ready slices, blocked tasks, and matching agents; the shared loop claims all independent ready slots it can safely route.
- Runesmith Worker Dispatch turns routed slots into executable worker packets with agent model policy, tool permissions, file scope, evidence requirements, lease/claim metadata, and a direct handoff, so OpenCode, CLI, and dashboard can drive independent slices without users handling task IDs. When the shared OS loop claims a packet for the user, it also focuses that packet automatically.
...
- Runesmith Decision Guard blocks autonomous Review or Seal decision synthesis when Review Lens or Seal Audit still has critical or blocking findings, returns a `decision-held` stop, and carries the exact guard findings back to OpenCode, CLI, dashboard, Runeweave, Loop Pulse, and Mission Memory.
...
- Runesmith Next is the hands-off router over the Runebook: one CLI command, one OpenCode tool, and one dashboard button that refines thin plans, proves, repairs, resolves supplied Faultline or risk decisions, recovers, or advances whichever card is active.
...
Runesmith Next is the default hands-off surface above the Runebook. It reads the active card and takes the smallest safe engine-owned action: refine a thin evidence-free Plan Contract, run Proof Runner for `Capture proof` and `Repair diagnostic`, stop at Faultline when repeated failed repairs need architecture review, apply supplied decisions for `Review faultline` and `Resolve risk`, recover stale work, claim the next dependency-ready task, or advance through Review and Seal when evidence is already present. If Review Lens or Seal Audit reports blocking findings, Next returns `decision-held` instead of synthesizing approval evidence. This is the Runesmith-owned version of workflow-skill dispatch: the user runs `runesmith next`, clicks `Run next`, or lets OpenCode call `runesmith_next`; the engine chooses the lower-level operation from runtime state.

Runeweave is the default OS run loop above Runesmith Next. It repeatedly executes engine-owned Runebook cards with a safety step limit, then stops with a concrete reason: sealed, idle, needs implementation evidence, proof failed, Faultline architecture review, risk held, decision held, blocked, or step limit reached. This is the boundary that keeps the product magical without faking autonomy: Runesmith handles leases, recovery, proof, decisions, Review, and Seal; the coding agent still performs creative implementation edits when the active card says `Continue forge`, and blocking Review/Seal findings stop as inspectable guard state instead of becoming false completion.
...
When the prepared mission is still a broad Forge/Review/Seal shell, OpenCode can call `runesmith_plan_refine` with concrete task slices, dependencies, capabilities, and required evidence. The dashboard can also run `Refine plan`; when no hand-authored task graph is supplied, Pathfinder Goal Weave profiles the mission goal and generates proof-backed install, runtime, UI, repair, docs, or focused Forge slices instead of forcing every project through a canned runtime/interface map. Normal users do not need to find that command: `runesmith_next`, `runesmith_os_run`, CLI `next`, CLI `run`, dashboard `Run next`, and dashboard `Run OS` pick the Plan Refinery card automatically while the Plan Contract is thin and evidence-free. Runesmith rejects unprovable slices, remaps the mission capsule, records the planning decision on the active root task, then runs the same evidence-gated loop. If the refined plan has independent ready implementation work, Dispatch Matrix and Agent Mesh claim those slices in the same pass.
...
After that, Runesmith listens to OpenCode tool execution. Shell commands become `command-output` evidence, passing verification commands such as test, typecheck, lint, and build become `test-result` evidence, failing verification commands become `diagnostic` evidence, and file-edit tools become `file-change` evidence on the active task. When a Worker Dispatch packet was claimed, replayed, or auto-claimed by the shared OS loop most recently, that packet becomes the focused evidence and proof target; captured tool evidence and Proof Runner evidence carry its packet, agent, holder, and lease metadata. Each captured evidence event runs the same evidence-gated advance loop, so a task can complete immediately after the required proof appears. The gate requires passing proof to be newer than the latest edit or diagnostic on that task, which keeps completion tied to the current work rather than earlier green runs. It also treats unresolved `risk` evidence as a human-hold gate that requires a later `decision`, so high-risk findings cannot be washed away by passing tests. When a planned task completes, Runesmith claims the next dependency-ready task automatically and focuses the returned worker packet for subsequent evidence. Covenant Review and Seal synthesize their own `decision` evidence only when Review Lens and Seal Audit are clear enough for autonomous approval; blocking findings return a decision guard instead. Routine missions can still finish end to end, while drift, stale proof, unresolved risk, and blocking completion findings stay visible.
...
- **Plan Refinery**: refine a thin mission into proof-backed slices through OpenCode or the dashboard `Refine plan` control, with Pathfinder Goal Weave recording the planning decision and claiming goal-matched ready slices automatically.
...
- `experimental.session.compacting`: appends the current mission capsule, Control Brief, Loop Pulse, Mission Map, Plan Contract, Dispatch Matrix, Worker Dispatch, Scope Sentinel, Redline Proof, Repair Contract, Review Lens, Seal Audit, Runebook, Mission Memory, and Proof Plan to compaction context with default Agent Mesh contracts registered in runtime.
...
- `Runesmith Worker Dispatch`: injected into system, message bootstrap, compaction context, dashboard state, and `runesmith_covenant_status` so OpenCode sees executable worker packets with agent, model, scope, evidence, lease, and claim metadata.
...
- `runesmith_worker_claim`: claim or re-focus the next or selected Worker Dispatch packet without raw mission/task ids, using the packet's assigned contract, holder, idempotency key, and lease TTL. This is an operator override; the shared OS loop also focuses packets it claims through `runesmith_next`, `runesmith_os_run`, idle Runeweave, or recovery. The focus event steers automatic OpenCode evidence capture to that packet until another worker packet is focused.
...
- `runesmith_proof_run`: execute the active Proof Plan inside OpenCode, record proof or diagnostics with focused worker metadata when present, and advance the mission when verification passes.
...
- `runesmith_covenant_status`: returns the installed Covenant, live Agent Mesh-backed Control Brief, Loop Pulse, Mission Map, Plan Contract, Dispatch Matrix, Worker Dispatch, Scope Sentinel, Redline Proof, Repair Contract, Review Lens, Seal Audit, Runebook card, Proof Plan, and active runes from the runtime capsule.
