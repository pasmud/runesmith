...
- Runesmith Plan Refinery lets OpenCode turn a thin mission into concrete proof-backed slices through `runesmith_plan_refine`; Runesmith Next and Runeweave also trigger that refinement automatically when the Plan Contract is thin and evidence-free, so users can just run the OS and let the engine decompose work before broad Forge starts.
