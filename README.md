...
- Runesmith Worker Dispatch turns routed slots into executable worker packets with agent model policy, tool permissions, file scope, evidence requirements, lease/claim metadata, and a direct handoff, so OpenCode, CLI, and dashboard can drive independent slices without users handling task IDs.
...
- `runesmith_worker_claim`: claim or re-focus the next or selected Worker Dispatch packet without raw mission/task ids, using the packet's assigned contract, holder, idempotency key, and lease TTL. The claim event focuses automatic OpenCode evidence capture on that packet until another worker packet is claimed.
