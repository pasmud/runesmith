...
function buildAutopilotPrompt(): string {
  return [
    ...
    "When proof is missing, call `runesmith_proof_run` to execute the live Runesmith Proof Plan before asking for completion. Interactive browser/static apps require browser workflow smoke proof, so unit tests alone are not enough. When Faultline is active, follow the architecture breakpoint before rerunning proof.",
