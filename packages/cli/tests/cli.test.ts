  ...
  test("status prints the install state and current Loop Pulse", async () => {
    ...
    expect(result).toEqual({
      ...
      stdout: [
        ...
        "seal audit: collecting-proof; 3 findings",
  ...
  test("mission inspect prints graph details from a snapshot", async () => {
    ...
    expect(result).toEqual({
      ...
      stdout: [
        ...
        "Findings: Run proof for task_alpha: bun test.; Redline Proof missing for task_alpha: implementation changed before focused failing proof was captured.; Missing test-result evidence for task_alpha.",
