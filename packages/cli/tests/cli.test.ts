  ...
  test("status prints the install state and current Loop Pulse", async () => {
    ...
    expect(result).toEqual({
      ...
      stdout: [
        ...
        "plan contract: ready; Plan contract ready for mission_alpha: 1 focused implementation slice is mapped with proof evidence.",
        "dispatch matrix: serial; Dispatch Matrix serial for mission_alpha: 1 dispatch slot is active or ready.",
        "worker dispatch: active; Worker Dispatch has 1 leased packet for mission_alpha.",
  ...
  test("status stays useful before bootstrap", async () => {
    ...
    expect(result).toEqual({
      ...
      stdout: [
        ...
        "proof plan: none",
        "mission map: none",
        "plan contract: idle; No mission map is active for a Plan Contract.",
        "dispatch matrix: idle; No active mission is ready for Dispatch Matrix.",
        "worker dispatch: idle; No active mission is ready for Worker Dispatch.",
  ...
  test("mission inspect prints graph details from a snapshot", async () => {
    ...
    expect(result).toEqual({
      ...
      stdout: [
        ...
        "- active root task_alpha: agent=agent_atlas; lease=lease_alpha; blockers=none",
        "Worker dispatch:",
        "Status: active",
        "Summary: Worker Dispatch has 1 leased packet for mission_alpha.",
        "- leased worker_mission_alpha_task_alpha_agent_atlas: agent=agent_atlas; model=anthropic/claude-sonnet-4.5; lease=lease_alpha; claim=none; evidence=file-change, test-result; scope=packages/**",
        "Scope sentinel:",
        ...
        "Findings: Missing test-result evidence for task_alpha.; Redline Proof missing for task_alpha: implementation changed before focused failing proof was captured.",
        "Seal audit:",
        "Summary: mission_alpha needs stronger proof before any completion claim.",
        "- proof-gate: attention - Run proof for task_alpha: bun test.",
        "- redline-gate: attention - Redline Proof missing for task_alpha: implementation changed before focused failing proof was captured.",
        "- production-readiness: attention - Missing implementation proof: task_alpha passing test-result.",
        "- review-gate: attention - mission_alpha review is waiting for proof on task_alpha.",
        "Findings: Run proof for task_alpha: bun test.; Redline Proof missing for task_alpha: implementation changed before focused failing proof was captured.; Missing test-result evidence for task_alpha.; Missing implementation proof: task_alpha passing test-result.",
        "Production seal:",
        "Summary: mission_alpha needs stronger production-readiness proof before completion.",
        "- implementation-evidence: attention - Missing implementation proof: task_alpha passing test-result.",
        "Findings: Missing implementation proof: task_alpha passing test-result.",
