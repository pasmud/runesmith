...
- Plan Refinery uses `runesmith_plan_refine` to replace a thin map with proof-backed slices, record the planning decision, and let the shared loop claim independent ready slices; Pathfinder Goal Weave can infer install, runtime, UI, repair, docs, or focused slices from the mission goal when the user does not provide a task graph.
...
- Worker Dispatch turns routed slots into executable packets with assigned agent, model, tool permissions, file scope, required evidence, lease/claim metadata, and a direct handoff. Claiming, replaying, or letting the shared OS loop claim a packet focuses later automatic OpenCode evidence capture on that packet until another worker packet is focused.
- Proof Plan chooses verification commands, including Runescope impacted tests from changed-file evidence when repository files are discoverable. When a worker packet is focused, Proof Plan and Proof Runner target that packet and carry its worker metadata into proof evidence.
...
- `runesmith_worker_claim` when an operator needs to override or re-focus a specific Worker Dispatch packet; routine `runesmith_next`, `runesmith_os_run`, idle Runeweave, and recovery claims focus their own packets automatically.
- `runesmith_plan_refine` when Plan Contract is `thin` and the work can be mapped into concrete proof-backed slices.
...
Completion requires current evidence in the runtime capsule. Passing proof must be newer than the latest relevant file change or diagnostic. Plan Contract is the planning-discipline signal: if it is `thin`, call `runesmith_plan_refine` with concrete proof-backed execution slices before broad autonomous work; if no task graph is supplied, trust Pathfinder Goal Weave to generate goal-matched slices from the mission text. If Plan Contract is `blocked`, add or inherit evidence obligations before claiming the map is actionable. Plan Refinery records the planning decision and re-enters the shared loop, so do not manually attach raw planning evidence unless the tool cannot infer it. Dispatch Matrix is the routing-discipline signal: claim only `ready` slots, preserve `active` leases, do not parallelize `blocked` slots, and use the recommended contract unless there is a stronger runtime reason. Worker Dispatch is the execution-discipline signal: execute packets through their assigned contract, respect packet file scope and evidence requirements, use `runesmith_worker_claim` only when you need an explicit operator claim or re-focus, and treat leased packets as already owned work. Engine-owned claims from `runesmith_next`, `runesmith_os_run`, idle Runeweave, and recovery focus the returned packet automatically, so ordinary OpenCode file and shell tool activity plus Proof Runner output is recorded on that packet's task with worker metadata. Redline Proof is the review-discipline signal: prefer a focused failing proof or proof-file edit before implementation changes, and treat a missing Redline signal as a Review Lens/Seal Audit finding. Repair Contract is the debugging-discipline signal: after failed proof, make one hypothesis-linked implementation change, then rerun the exact failing command before broad verification; broad or blind repair edits become Review Lens/Seal Audit findings. When Proof Plan includes a `Run impacted test` command, run it before broad verification; the engine selected it from captured file changes. Unresolved risk requires later decision evidence. Stale leases must be recovered before unrelated work continues.
