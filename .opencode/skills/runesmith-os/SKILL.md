...
- Worker Dispatch turns routed slots into executable packets with assigned agent, model, tool permissions, file scope, required evidence, lease/claim metadata, and a direct handoff. Claiming or replaying a packet focuses later automatic OpenCode evidence capture on that packet until another worker packet is claimed.
...
- `runesmith_worker_claim` when Worker Dispatch has a claimable packet and the next worker should be leased without raw mission or task ids.
